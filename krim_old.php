...
<br>Du er ute kl. ' . date("H:i:s d.m.Y", (time() + $bu))) .
    ...
    if ($q->fetchColumn() == 1) {
        ...
        $q2 = $db->prepare("SELECT * FROM `krimlogg` WHERE `uid` = ? AND `timewait` > UNIX_TIMESTAMP() ORDER BY `timewait` DESC LIMIT 0,1");
        $q2->execute([$obj->id]);
        $f = $q2->fetchObject();
        if (time() < $f->timewait) {
            echo '
            ...
            if (!$jailed) {
                ...
                <form name="krim"
                    ...
                    <table style="width:590px;"
                        ...
                        $rank = rank($obj->exp);
                        $ranknr = $rank[0];
                        $sql = $db->prepare("SELECT * FROM `crime` WHERE `levelmin` <= '$ranknr' ORDER BY `levelmin` DESC,`id` DESC");
                        if ($db->num_rows() >= 1) {
