...
if($db->num_rows($g) >= 1){
  ...
  if($a['leder'] == $obj->id) {
    ...
    if ($leder || $a["spillervapen$spiller"] != 0) { // har kjøpt eller er leder  
      if($_POST['forlat']){
        if($leder){
          $db->query("UPDATE `ran` SET `active` = '0' WHERE `id` = '{$a['id']}'");
          $db->query("UPDATE `raninv` SET `active` = '0' WHERE `ranid` = '{$a['id']}'");
          echo '<p class="lykket">Du har forlatt ranet, alle som var med mistet utstyret sitt!</br>Oppdater siden for å komme deg bort.</p>';
        }
        else{
          $db->query("UPDATE `ran` SET `spiller$spiller` = '0',`spillervapen$spiller` = '0' WHERE `id` = '{$a['id']}'");
          echo '<p class="lykket">Du har forlatt ranet! <a href="/Ran">Klikk her for å gå tilbake til Ran.</a></p>';
        }
      }
      ...
      <form action="" method="POST">
        <table style="width:585px;" class="table">
          ...
          echo '
            ...
            <tr>
              <td>'.($a['spiller1'] == 0 ? 'Venter på spiller..' : user($a['spiller1'])).'<a href="Ran?kast='.$a['spiller1'].'&s1">Kast ut!</a></td>
              <td>'.($a['spillervapen1'] == 0 ? 'Har ikke kjøpt' : $utstyr[$a['spillervapen1']]['name']).'</td>
            </tr>
            <tr>
              <td>'.($a['spiller2'] == 0 ? 'Venter på spiller..' : user($a['spiller2'])).'<a href="Ran?kast='.$a['spiller2'].'&s2">Kast ut!</a></td>
              <td>'.($a['spillervapen2'] == 0 ? 'Har ikke kjøpt' : $utstyr[$a['spillervapen2']]['name']).'</td>
            </tr>
            <tr>
              <td>'.($a['spiller3'] == 0 ? 'Venter på spiller..' : user($a['spiller3'])).'<a href="Ran?kast='.$a['spiller3'].'&s3">Kast ut!</a></td>
              <td>'.($a['spillervapen3'] == 0 ? 'Har ikke kjøpt' : $utstyr[$a['spillervapen3']]['name']).'</td>
            </tr>
            <tr>
              <td>'.($a['spiller4'] == 0 ? 'Venter på spiller..' : user($a['spiller4'])).'<a href="Ran?kast='.$a['spiller4'].'&s4">Kast ut!</a></td>
              <td>'.($a['spillervapen4'] == 0 ? 'Har ikke kjøpt' : $utstyr[$a['spillervapen4']]['name']).'</td>
            </tr>'; 
?>          
        </table>
        <?php if($leder)echo '<input type="submit" name="fullfor" value="Fullfør ranet!">';?>
        <input type="submit" name="forlat" onclick="return confirm('Er du sikker på at du vil forlate ranet?')" value="Forlat Ranet!">
      </form>
      <?php
      if($leder){ // Om leder.. kan invitere og fullføre ranet.
        ?>
</br>
<form action="" method="POST">
...
<?php
      if($_POST['fullfor']){
        ...
        else if($allevapen == false) echo '<p class="feil">Ikke alle har utstyr ennå.</p>';
        if ($allespillere && $allevapen) {// utfør ranet
          $rand = mt_rand(0,100);
...
//          echo count($types);
          if($rand <= 50 ){
            $bel = 0;
            $timeleft = time() + 900;
            echo '<p class="feil">Ranet var ikke vellykket! Dere ble tatt av purken og sitter nå i fengsel!</p>';
          ...
          elseif($rand >= 50){
            ...
            $lykket = array(
                1=>"Dere plaffet ned alle dere så, og fikk med dere",
                2=>"Dere møtte noen hindringer, men klarte å skremme alle nok til å ikke ringe purken!</br>Dere fikk med dere",
                3=>"Moren din ringte mens du var i ranet. Heldigvis hadde du handsfree og møtte ingen flere hindringer!</br>Dere fikk med dere",
                4=>"Daglig leder hadde glemt å låse safen. Dette var lett!</br>Dere fikk med dere"
            );
          ...
          $db->query("UPDATE `users` SET `exp` = (`exp` + 7.5) WHERE `id` IN(".$a['spiller1'].",".$a['spiller2'].",".$a['spiller3'].",".$a['spiller4'].")");/*Gir de som var med på ranet exp.*/
          sysmel(array($a['spiller1'],$a['spiller2'],$a['spiller3'],$a['spiller4']), 'Ran -- Vellykket ran!<br>Dere fikk med dere <b>'.number_format().'</b>kr! Du fikk også litt XP!');
          }
    ...
    } else { // ikke kjøpt ... heller ikke leder
      
      
      if(isset($_POST['submitteren'])){// kjøper utstyr...
        $id = intval($_POST['utstyrvelg']);
        if(isset($utstyr[$id]['name'])){
          if($obj->hand < $utstyr[$id]['pris']){
            echo '<p class="feil">Du har ikke råd til dette!</p>';
          }else{
    ...
    echo '<p class="lykket">Du har et ran ventende på deg, eller du har ikke valgt utstyr!</p>';
?>
<form action="" method="POST">
  <table style="text-align:center;" class="table">
    ...
    ?>
    <tr><td><input type="submit" name="submitteren" value="Kjøp Utstyr!"></td></tr>
  </table>
</form>
<?php
    ...
    if($db->num_rows($kla) >= 1){
      ...
      if($db->num_rows() == 1){
        ...
        if($obj->city == $b['by']){
          ...
          echo '<p class="feil">Du må være i samme by som ranet ble startet!<br>Ranet befinner seg i '.city($b['by'],1).', mens du er i '.city($obj->city,1).'.</p>';
        }
      }
      else{
        echo '<p class="feil">Beklager, men det ser ut som ranet var utført før du kunne bli med!</p>';
      }
    }
    else{
...
if(isset($_GET['ran'])){ // Starter ett nytt ran ...
  ...
  if($id == 1){
    if($obj->exp < 100){
      echo '<p class="feil">Du må være Underboss eller høyere for å starte dette ranet!</p>';
    }
    else{
      if($obj->hand >= 1000000){
  ...
  if($id == 2){
    if($obj->exp < 350){
      echo '<p class="feil">Du må være Don eller høyere for å starte dette ranet!</p>';
    }
    else{
      if($obj->hand >= 4000000){
...
<form>
  <table style="width: 330px;"class="table">
    ...
    <tr><td><a href="/Ran?ran=1" onclick="return confirm('Ved å starte dette ranet vil du bruke 1,000,000kr på å starte planleggingen, godta?');">Ran den lokale 7-11</a></td><td>Underboss</td><td>1,000,000kr</td></tr>
    <tr><td><a href="/Ran?ran=2" onclick="return confirm('Ved å starte dette ranet vil du bruke 4,000,000kr på å starte planleggingen, godta?');">Ran NOKAS</a></td><td>Don</td><td>4,000,000kr</td></tr>
  </table>
</form>
