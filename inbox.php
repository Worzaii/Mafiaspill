...
if (!isset($_GET['do'])) {
    ...
    ';
    $sql = mysql_query(
        "SELECT * FROM `mail` WHERE `TId` = '$obj->id' OR `UId` = '$obj->id' GROUP BY `theme` ORDER BY `time` DESC"
    ) or die('Feil: ' . mysql_error());
                ...
                if (strlen($usto) <= 3) {
                    ...
                    if (strlen($tema) == 0) {
                        ...
                        if (mysql_num_rows($q) == 1) {
                            ...
                            if (mysql_query(
                                "INSERT INTO `mail`(`id`,`UId`,`TId`,`theme`,`message`,`time`,`date`) VALUES(NULL,'$obj->id','$u->id','$tema','$sms','$time','$dato')"
                            )) {
            ...
            if (!is_numeric($id)) {
                ...
                if ($id <= 0) {
                    ...
                    if (is_numeric($id) && $id >= 1) {
                        $sql = mysql_query(
                            "SELECT * FROM `mail` WHERE `UId` = '$obj->id' AND `id` = '$id' OR `TId` = '$obj->id' AND `id` = '$id'"
                        ) or die(mysql_error());
                        $sql2 = mysql_fetch_object($sql) or die(mysql_error());
                        $sql3 = mysql_query(
                            "SELECT * FROM `mail` WHERE `theme` = '$sql2->theme' AND `UId` = '$obj->id' OR `theme` = '$sql2->theme' AND `TId` = '$obj->id' ORDER BY `id` DESC"
                        ) or die(mysql_error());
                        if (!mysql_query(
                            "UPDATE `mail` SET `read` = '1' WHERE `theme` = '$sql2->theme' AND `TId` = '$obj->id'"
                        )) {
                            if ($obj->status == 1) {
                        ...
                        if (isset($_POST['svar1'])) {
                            ...
                            $dato = date("H:i:s d.m.Y");
                            if (mysql_query(
                                "INSERT INTO `mail`(`id`,`UId`,`TId`,`theme`,`message`,`time`,`date`) VALUES(NULL,'$obj->id','$till','$tema','$svar','$time','$dato')"
                            )) {
                                echo lykket('Meldingen er blitt sendt!');
                            } else {
                                if ($obj->status == 1) {
