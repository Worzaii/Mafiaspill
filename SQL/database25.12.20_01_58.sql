...
GRANT ALL ON mafia.* TO 'mafia'@'localhost'; /* Fixed a silly typo, ops */
FLUSH PRIVILEGES;
...
/*!40000 ALTER TABLE `bank_transfer`
    DISABLE KEYS */;
/*!40000 ALTER TABLE `bank_transfer`
    ENABLE KEYS */;
UNLOCK TABLES;
...
/*!40000 ALTER TABLE `lotteryconfig`
    DISABLE KEYS */;
/*!40000 ALTER TABLE `lotteryconfig`
    ENABLE KEYS */;
UNLOCK TABLES;
...
/*!40000 ALTER TABLE `lotterytickets`
    DISABLE KEYS */;
/*!40000 ALTER TABLE `lotterytickets`
    ENABLE KEYS */;
UNLOCK TABLES;
...
/*!40000 ALTER TABLE `stillingslogg`
    DISABLE KEYS */;
/*!40000 ALTER TABLE `stillingslogg`
    ENABLE KEYS */;
UNLOCK TABLES;
...
/*!50001 CREATE ALGORITHM = UNDEFINED */ /*!50013 DEFINER =`root`@`localhost` SQL SECURITY DEFINER */ /*!50001 VIEW `LessUser` AS
select `users`.`id`          AS `id`,
       ...
       `users`.`hand`        AS `hand`,
       `users`.`city`        AS `city`,
       `users`.`weapon`      AS `weapon`,
       `users`.`bullets`     AS `bullets`,
       `users`.`health`      AS `health`,
       `users`.`points`      AS `points`,
       `users`.`exp`         AS `exp`,
       `users`.`status`      AS `status`,
       `users`.`support`     AS `support`,
       `users`.`ip`          AS `ip`,
       `users`.`regip`       AS `regip`,
       `users`.`hostname`    AS `hostname`,
       `users`.`reghostname` AS `reghostname`,
       `users`.`lastactive`  AS `lastactive`,
       `users`.`forceout`    AS `forceout`,
       `users`.`regstamp`    AS `regstamp`,
       `users`.`picmaker`    AS `picmaker`
from `users`
*/;
