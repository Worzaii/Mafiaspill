...
if(r1()){
  ...
  . 'blir vist øverst, og de med færre nedover. '
  . 'For å finne en spesifikk bruker, skriv inn i tekstfeltet og trykk enter(linjehopp).</p>';
  $q = $db->query("SELECT `uid`,COUNT(*) AS `sum` FROM `sessusr` GROUP BY `uid` ORDER BY COUNT(*) DESC LIMIT 0,40");
//  $pag = new Pagination($db, $q, 40, "s");
  echo '<p>Antall rader å vise: '.$db->num_rows().'</p>';
  if($db->num_rows() >= 1){
    ...
    echo '<p class="feil">Merkelig, det er ingen innlogginger å vise :S</p>';
