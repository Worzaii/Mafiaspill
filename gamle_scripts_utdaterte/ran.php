    ...
    if ($db->num_rows($g) >= 1) {
        ...
        if ($leder || $a["spillervapen$spiller"] != 0) { // har kjøpt eller er leder
            if ($_POST['forlat']) {
                if ($leder) {
                    ...
                    echo lykket('Du har forlatt ranet, alle som var med mistet utstyret sitt!</br>Oppdater siden for å komme deg bort.');
                } else {
                    $db->query("UPDATE `ran` SET `spiller$spiller` = '0',`spillervapen$spiller` = '0' WHERE `id` = '{$a['id']}'");
                    echo lykket('Du har forlatt ranet! <a href="/Ran">Klikk her for å gå tilbake til Ran.</a>');
                }
            }
      ...
      <form action="" method="POST">
        <table style="width:585px;" class="table">
          ...
          <?php if ($leder) {
              echo '<input type="submit" name="fullfor" value="Fullfør ranet!">';
          } ?>
    ...
    <input type="submit" value="Inviter spiller!">
                ...
                if ($_POST['fullfor']) {
                    ...
                    if ($allespillere == false) {
                        ...
                        echo feil('Ikke alle har utstyr ennå.');
                    }
                    if ($allespillere && $allevapen) {// utfør ranet
                        ...
                        if ($rand <= 50) {
                            $bel = 0;
                            $timeleft = time() + 900;
                            echo feil('Ranet var ikke vellykket! Dere ble tatt av purken og sitter nå i fengsel!');
            ...
            if (isset($_POST['submitteren'])) {// kjøper utstyr...
                ...
                if (isset($utstyr[$id]['name'])) {
                    if ($obj->hand < $utstyr[$id]['pris']) {
                        echo feil('Du har ikke råd til dette!');
                    } else {
            ...
            echo lykket('Du har et ran ventende på deg, eller du har ikke valgt utstyr!');
            ?>
<form action="" method="POST">
  <table style="text-align:center;" class="table">
    ...
    <tr><td><input type="submit" name="submitteren" value="Kjøp Utstyr!"></td></tr>
                ...
                if ($db->num_rows($kla) >= 1) {
                    ...
                    if ($db->num_rows() == 1) {
                        ...
                        if ($obj->city == $b['by']) {
                            ...
                            echo feil('Du må være i samme by som ranet ble startet!<br>Ranet befinner seg i ' . city($b['by'], 1) . ', mens du er i ' . city($obj->city, 1) . '.');
                        }
                    } else {
                        echo feil('Beklager, men det ser ut som ranet var utført før du kunne bli med!');
                    }
                } else {
        ...
        if (isset($_GET['ran'])) { // Starter ett nytt ran ...
            ...
            if ($id == 1) {
                if ($obj->exp < 100) {
                    echo feil('Du må være Underboss eller høyere for å starte dette ranet!');
                } else {
                    if ($obj->hand >= 1000000) {
            ...
            if ($id == 2) {
                if ($obj->exp < 350) {
                    echo feil('Du må være Don eller høyere for å starte dette ranet!');
                } else {
                    if ($obj->hand >= 4000000) {
