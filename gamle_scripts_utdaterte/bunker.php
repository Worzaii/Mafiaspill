    ...
    if ($obj->eigendom == 0) {
        ...
        if (isset($_POST['uinv']) && isset($_POST['ubtid']) && isset($_POST['sendbunker'])) {
            ...
            for ($o = 0; $o < count($names); $o++) {
                $t = $db->query("SELECT * FROM `bunkerinv` WHERE `tid` = '" . getUser($names[$o]["i"], 1) . "' AND `uid` = '{$obj->id}' AND `used` = '0' AND `accepted` = '0' AND `declined` = '0' AND `gone` = '0'");
                if ($db->num_rows() >= 1) {/*Spiller har allerede fått bunker*/
                    echo feil('Kan ikke sende bunker til ' . user(getUser($names[$o]["i"], 1)) . ' da du allerede har invitert spilleren!');
                } else {
                    if (getUser($names[$o]["i"], 1) == $obj->id) { /*Kan ikke sende til seg selv*/
                        echo feil('Du kan ikke sende bunker til deg selv!');
                    } else {
                        if ($db->query("INSERT INTO `bunkerinv`(`uid`,`tid`,`time`,`timeleft`,`length`) "
                            . "VALUES('" . $obj->id . "','" . getUser($names[$o]["i"], 1) . "',UNIX_TIMESTAMP(),NULL,'" . $names[$o]['t'] . "')")) {
                            echo lykket('' . htmlentities($names[$o]["i"]) . ' mottok bunkerinvitasjon på ' . $dont_touch_my_tralala[$names[$o]['t']] . '!');
                        } else {
            ...
            if (is_numeric($id) && $id >= 1) {
                ...
                echo feil('Feil valg, prøv igjen!');
            }
        }
    }
  /*Viser tabeller osv*/
        ...
        if (isset($_POST['godtainv'])) {
            ...
            if ($db->num_rows() == 1) {
                ...
                if ($db->query("UPDATE `bunkerinv` SET `accepted`='1',`timeleft`='" . (time() + $o1->length) . "',`used`='1' WHERE `id` = '$id'")) {
                    echo lykket('Du har nå satt deg i bunker i ' . $times[$o1->length] . '!<br>Du er ute om: <span id="tidforute"></span><script>teller(' . (time() + $o1->length) . ')</script>');
                } else {
