            ...
            if (isset($_POST['smsback'])) {
                ...
                if ($r->besvart == 1) {
                    ...
                    if ($db->query("INSERT INTO `mail2`(`supportid`,`tid`,`fid`,`title`,`message`,`time`) VALUES('{$r->id}','$r->usid','$obj->id','Support svar: $r->theme','$sms',UNIX_TIMESTAMP())")) {
                        ...
                        $db->query("UPDATE `support` SET `read` = '1',`besvart` = '1',`besvartav` = '{$obj->id}' WHERE `id` = '{$r->id}' LIMIT 1");
        ...
        <td colspan="2"><b>Melding:</b> <br>'.$textout.'</td>
				...
				<form method="post" action="">
					<table class="table">
						...
						<tbody>
							<tr>
								<td style="text-align:center;"><textarea style="width:100%;height:230px;color:#aaa;background-color:#333;border:0px;" name="smsback" placeholder="Skriv svaret her">'.htmlentities($textin->message,
                NULL, "ISO-8859-1").'</textarea><br><input type="submit" value="Besvar meldingen!"></td>
                                                              ';
            if ($r->read == 1) {
                \NULL;
            } else {
            ...
            if ($r->besvart == 1) {
                echo '<p>Denne meldingen ble besvart av: '.user($besav->besvartav).'</p>';
            }
            echo '
							</tr>
						</tbody>
      ...
      ';
        ...
        if ($db->num_rows($getmes) == 0) {
            ...
            while ($r = mysqli_fetch_object($getmes)) {
                ...
                echo '
        <tr>
        <td><a href="supportpanel.php?vis='.$r->id.'">'.$ny1.check($r->theme).'</a></td><td>'.user($r->usid).'</td><td>'.date("H:i:s d.m.y",
                    $besav->time).'</td><td>'.$besvart.'</td>
        </tr>
        ';
            }
        }
