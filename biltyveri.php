...
if ($write = canUseFunction(1, 1)) {
    ...
    $q = $db->query(
        "SELECT * FROM `carslog` WHERE `uid` = '$obj->id' AND `timewait` > UNIX_TIMESTAMP() ORDER BY `id` DESC LIMIT 0,1"
    );
    if ($q->num_rows() == 1) {
            ...
            if (is_numeric($v) && $v >= 1) {
                ...
                if ($db->num_rows() == 1) {
                    ...
                    if ($se->bilmin > $se->bilmax || $se->bilmin < 1) {
                        ...
                        $sa = $db->query(
                            "SELECT * FROM `chance` WHERE `type` = '2' AND `uid` = '$obj->id' AND `option` = '$v'"
                        );
                        if ($db->num_rows() == 1) {
                            $si = $db->fetch_object();
                            if (rand(1, 100) > $si->chance) {
                                if (rand(1, 2) == 1) {
                                    ...
                                    if (!$db->query(
                                        "INSERT INTO `carslog`(`uid`,`timestamp`,`timewait`,`result`,`choice`) VALUES('$obj->id',UNIX_TIMESTAMP(),'" . (time(
                                            )
                                            + $se->timewait) . "','0','$v')"
                                    )) {
                                        echo feil(
                                            'Det var feil med en spørring, og det ble lagret i loggen.
                                    ...
                                    if (!$db->query(
                                        "INSERT INTO `carslog`(`uid`,`timewait`,`timestamp`,`result`,`choice`) VALUES('{$obj->id}','" . (time(
                                            )
                                    ...
                                    if ($db->query(
                                        "INSERT INTO `jail`(`uid`,`reason`,`timestamp`,`timeleft`,`priceout`) VALUES('{$obj->id}','Prøvde &aring stjele bil',UNIX_TIMESTAMP(),'" . (time(
                                            )
                                            + $se->punishtime) . "', 5000)"
                                    )) {
                                        echo feil(
                                ...
                                if (!is_array($cartypes)) {
                                    echo feil(
                                        'Det er en feil i bilconfig! Vennligst rapporter dette til Admin snarest!'
                                    );
                                } else {
                                    $whatcar = (rand($se->bilmin, $se->bilmax) - 1); //Velger biler
                                    echo lykket(
                                        'Du fikk med deg ' . $cartypes[$whatcar]["name"] . ' med en verdi p&aring ' . number_format(
                                            $cartypes[$whatcar]["price"]
                                        ) . '!'
                                    );
                                    if ($db->query(
                                        "INSERT INTO `garage`(`car_id`,`uid`,`stolen_city`,`current_city`,`timestamp`) VALUES('{$whatcar}','{$obj->id}','{$obj->city}','{$obj->city}',UNIX_TIMESTAMP())"
                                    )) {
                                        if ($db->query(
                                            "INSERT INTO `carslog`(`uid`,`timestamp`,`timewait`,
                      `result`,`choice`)
...
WHERE `id` = '{$obj->id}' LIMIT 1"
                            ...
                            if ($si->chance >= 74) {
                                $ran2 = rand(10, 36);
                                $db->query(
                                    "UPDATE `chance` SET `chance` = (`chance` - $ran2)
...
WHERE `uid` = '$obj->id' AND
      `option` = '$si->option' LIMIT 1"
                            ...
                            echo feil(
                                'Beklager, det kan se ut som det er feil med din sjanse-sjekker. Ta kontakt med admin for å få dette fikset.'
                            );
                        }
                    }
                } else {
            ...
            <div id="biltyveri">
                <form method="post" id="bil" action="">
                    <table class="table" style="width:590px;">
                        ...
                        $rank = rank($obj->exp);
                        $s = $db->query(
                            "SELECT * FROM `cars` WHERE `levelmin` <= '" . $rank[0] . "'
