...
include("core.php"); 
startpage("Biltyveri");
echo '<img src="imgs/biltyveri.png" /><p>Når du først starter med biltyveri, så vil du kun ha et valg. Ettersom du kommer opp i rank, så vil nye valg låses opp.</p>';
if(fengsel() == true){
...
die();
  ...
  if($db->num_rows() == 1){
    ...
    <script type="text/javascript">teller('.$left.',"tid",true,"ned");</script>';
  }
  else{
          ...
          if($se->bilmin > $se->bilmax || $se->bilmin < 1){
            ...
            if($db->num_rows() == 1){
              ...
              if(rand(1,100) > $si->percent){
                //Om random tall er høyere en sjanse, så fail
                if(rand(1,2) == 1){
                  ...
                  if(!$db->query("INSERT INTO `billogg`(`uid`,`time`,`timelast`,`resu`,`bvalg`) VALUES('$obj->id','$time','".time()."','0','$v')")){
                    if($obj->status == 1){
                      ...
                      echo '<p>Det var feil med en spørring, og det ble lagret i loggen. Varsle Ledelsen om dette snarest, slik at de kan se igjennom det.</p>';
                    }
                  }
                }
                else{
                  echo '<p class="feil">Du klarte det ikke og havnet i fengselet! Går til fengsel om 3 sekunder</p>';
                  $time = time();
                  $time2 = time() + 90;
				 ...
				 header('Refresh: 3;url=fengsel.php');
                  ...
                  if($db->query("INSERT INTO `jail`(`uid`,`reason`,`time`,`timeleft`) VALUES('$obj->id','Pr&oslash;vde &aring stjele bil','$time','$time2')")){
                    echo '<p class="feil">Tid igjen: <span id="fengsel"></span><script type="text/javascript">teller(90,"fengsel",true,"ned");</script></p>';
                  }
                }
              }
              else{
              ...
              feil('
              <p>Beklager, det kan se ut som det er feil med din sjanse-sjekker. Ta kontakt med admin for å få dette fikset.</p>
              ');
            }
          }
        }
