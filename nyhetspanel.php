...
if (r1() || r2()) {
    ...
    while ($new = $news->fetchObject()) {
        $rows .= "<tr id='tr{$new->id}'><td>{$new->id}</td><td>{$new->title}</td><td>{$new->userlevel}</td><td>{$new->showing}</td><td>
<button onclick='return choice({$new->id},1)'>Endre</button><button onclick='return deletenews({$new->id},2)'>Slett</button><button onclick='return choice({$new->id},3)'>Vis/skjul</button>
</td></tr>";
    }
