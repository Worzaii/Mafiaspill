...
?>
    ...
    <table class="table" style="width:300px;">
        <tr>
            <td>Ipadresse:</td>
            <td style="width:300px"><?= $_SERVER['REMOTE_ADDR'] ?></td>
        </tr>
        <tr>
            <td>Rank:</td>
            <td style="width:300px"><?= $rank[1] ?>(<?= $rank[0] ?>)</td>
        </tr>
    </table>
<?php
$fuck = $db->query("SELECT * FROM `krimlogg` WHERE `uid` = '$obj->id'");
$krlyk = $db->num_rows($db->query("SELECT * FROM `krimlogg` WHERE `uid` = '$obj->id' AND `result` = '1'"));
$krfeil = $db->num_rows($db->query("SELECT * FROM `krimlogg` WHERE `uid` = '$obj->id' AND `result` = '0'"));
$fuckxto = $db->query("SELECT * FROM `carslog` WHERE `uid` = '$obj->id'");
$fuckxtolyk = $db->num_rows($db->query("SELECT * FROM `carslog` WHERE `uid` = '$obj->id' AND `result` = '1'"));
$fuckxtofeil = $db->num_rows($db->query("SELECT * FROM `carslog` WHERE `uid` = '$obj->id' AND `result` = '0'"));
$fuckxtofeilto = $db->num_rows($db->query("SELECT * FROM `carslog` WHERE `uid` = '$obj->id' AND `result` = '2'"));
...
?>
    ...
    <div style="margin-left: auto;margin-right: auto;width: 550px;height: 50px">
        <div style="width:550px;margin-top:10px;text-align: center;height: 40px;background: #501;position: absolute;z-index: 1;border-radius: 16px;overflow: hidden;border: 2px solid #000;">
            <div style="background: #22d300;width: <?= number_format(
                $prosent,
                1
            ) ?>%;height: 40px;border-radius: 0px;"></div>
        </div>
        <p style="padding:0;z-index: 2;position: relative;margin:0;top:22px;text-align: center"><?= number_format(
            $prosent,
            3
        ) ?>%</p>
    </div>
