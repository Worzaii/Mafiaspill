            ...
            if ($db->num_rows() == 1) {
                ...
                if ($uid->pass == $pa) {
                    if ($uid->health > 0 && $uid->moddet == 0) {
                        $str                    = ['string' => "Innlogget! Et lite &oslash;yeblikk imens vi sender deg inn til nyheter...",
                        ...
                        $db->query("INSERT INTO `sessusr`(`uid`,`user_agent`,`ip`,`ip_host`,`security_key_gen`,`time`) VALUES('$uid->id','".$_SERVER['HTTP_USER_AGENT']."','".$_SERVER['REMOTE_ADDR']."','".gethostbyaddr($ip)."','".safegen($uid->user,
                                $uid->pass)."',UNIX_TIMESTAMP())"); /* Legger inn sikkerhetsnøkkel unik for nåværende spiller. */
                    } else {
                        if ($uid->moddet == 1) {
                            $str = array('string' => "<p class='feil'>$uid->user har blitt modkillet av {$uid->modav}.<br />Grunnlag: ".gen($uid->modgrunn)."</p>",
                                "state" => 0);
                        } elseif ($uid->health == 0) {
                            $str = array('string' => gen("<p class='feil'>Du har blitt drept! For å spille, registrer en ny bruker!</p>"),
                                "state" => 0);
                        }
                    }
                } else {
                    //Feil passord
                    $str = array('string' => "<p class='feil'>Brukernavnet eller passordet er ikke rett, prøv igjen!</p>",
                        "state" => 0);
                }
            } else {
...
if (isset($_GET['gpw'])) {
    ...
    if (strlen($user) >= 4 && strlen($user) <= 20 && filter_var($mail, FILTER_VALIDATE_EMAIL)) {
        ...
        if ($db->num_rows() == 1) {
            ...
            $db->query("INSERT INTO `resetpasset`(`uid`,`resgen`,`timemade`,`rimming`) VALUES('".$i->id."','".$resgen."','".time()."','".(time()
                + 3600)."')");
            $message = '
      <html>
      <head>
...
if (isset($_GET['respas'])) {
    ...
    if (strlen($p1) >= 4 && ($p1 == $p2) && is_numeric($uid)) {
        ...
        if ($db->connection_id) {
            ...
            if ($db->num_rows() == 1) {
                ...
                if ($db->num_rows() == 1) {
                    ...
                    if ($db->affected_rows() == 1) {
                        ...
                        $str = array('string' => gen('<p style="color:#50a850;">Ditt passord har blitt endret! Du kan nå logge inn på innloggingssiden med det nye passordet ditt!</p>'),
                            'res' => 1);
                        ...
                        $str = array('string' => gen('<p style="color:#f00;">Kunne ikke oppdatere passordet! 2 muligheter står:<br>Du prøvde å bruke samme passordet<br>Det var en feil i query til databasen! <br>Send en mail til baretester@live.no om problemet redvarer!</p>'),
                            'res' => 0);
                    }
                } else {
            ...
            $str = array('string' => gen('<p style="color:#f00;">Databasen er ikke tilgjengelig! Prøv igjen senere!</p>'),
                'res' => 0);
        }
    } else {
        /* Validering slo ut på false på en eller flere valideringer */
        $str = array('string' => gen('<p style="color:#f00">Passordet ditt må være 4 tegn eller lengre, og være like i begge feltene under! Det kan også være at ikke riktig uid ble postet.</p>'),
            'res' => 0);
    }
