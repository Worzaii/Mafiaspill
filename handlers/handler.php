...
END;
    ...
    else if(isset($_GET['reg']))
    {
        ...
        if (isset($_POST['user'])) 
        {
            ...
            $res = null;
            if($rad >= 1 || $rad2 >= 1 || strlen($user) <= 3 || strlen($user) >= 16 || !preg_match("/^[a-zA-Z]+[\w-_]*$/i", $user) || !filter_var($mail, FILTER_VALIDATE_EMAIL) || $pas1 != $pas2 || $image->check($_POST['captcha_code']) == false)
            {
                $res .= '
