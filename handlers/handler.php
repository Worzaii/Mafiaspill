...
if (!(isset($_GET['login']) ||
    isset($_GET['getaccess']) ||
    isset($_GET['brukerreg']) ||
    isset($_GET['forgotpassword']) ||
        ...
        if (strlen($_POST['username']) === 0 || strlen($_POST['password']) === 0) {
            ...
            if ($st->fetchColumn() === 1) {
                ...
                if (password_verify($pa, $uid->pass) && $uid->health > 0) {
                    ...
                    $st3 = $db->prepare("insert into sessions(uid, user_agent, user_ip, timestamp) " .
                        "VALUES (?, ?, ? ,UNIX_TIMESTAMP())");
                    $st3->execute([$uid->id, $_SERVER["HTTP_USER_AGENT"], ip2long($ip)]);

                    $st4 = $db->prepare("UPDATE `users` SET `lastactive` = UNIX_TIMESTAMP()," .
                        "`ip` = ?, `hostname` = ? where `id` = ? AND `pass` = ?");
...
if (isset($_GET['resetpassword'])) {
    ...
    if (strlen($p1) >= 4 && ($p1 == $p2) && is_numeric($uid)) {
        ...
        $s = $db->prepare("SELECT count(*) FROM `resetpasset` WHERE `uid` = ? AND resgen = ? AND `used` = '0'
        ...
        if ($s->fetchColumn() == 1) {
            /**
             * Reset code found, continuing to update user password
             */
...
WHERE `uid` = ? AND resgen = ? AND `used` = '0' ORDER BY `id` DESC LIMIT 1");
                    $respw->execute([$uid, $resgen]);
                    if ($respw->rowCount() == 1) {
                        $str['string'] = lykket('Passordet er oppdatert! ' .
                            'G&aring; til innlogging for &aring; fortsette!');
                    } else {
...
<br>Send en mail til ' . HENVEND_MAIL . ' om problemet redvarer!');
        ...
        $str['string'] = feil('Passordet ditt må være 4 tegn eller lengre,' .
            'og være like i begge feltene under! Det kan også være at ikke riktig uid ble postet.');
    }
}
