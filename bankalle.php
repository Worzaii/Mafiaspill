...
if ($obj->status == 1 || $obj->status == 2) {
  ...
  <th>Id</th><th>Fra</th><th>Til</th><th>Sum:</th><th style="width:101px">Tid:</th>
    ...
    $sql = "SELECT * FROM `bank_transfer`  ORDER BY `id` DESC";
    ...
    foreach ($bank as $tall) {
        echo '
    <tr>
    <td style="font-size:20px;">#' . $tall['id'] . '<td>' . status($tall['uid'], 1) . '</td><td>' . status($tall['tid'],
                1) . '</td><td>' . number_format($tall['sum']) . 'kr</td><td>' . date("H:i:s d.m.y", $tall['time']) . '</td>
    </tr>
    ';
    }
