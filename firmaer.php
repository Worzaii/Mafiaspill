...
';
    ...
    if (bunker() == true) {
        $bu = bunker(true);
        echo /** @lang javascript */
            feil('Du er i bunker, gjenst&aring;ende tid: <span id="bunker">' . $bu . '</span><br>Du er ute kl. ' . date("H:i:s d.m.Y",
                    $bu)) . '
<script>
...
';
        ...
        $s = $db->query("SELECT * FROM `firms` WHERE `uid` = '{$obj->id}' ORDER BY `id` ASC");//Henter firmapanelene til spilleren.
        if ($db->num_rows() >= 1 || $obj->status == 1) {
            ...
            if (isset($_POST['allout']) || isset($_POST['overfor'])) {
                if (isset($_POST['allout'])) {
                    ...
                    if (isset($_POST['overfor']) && isset($_POST['overfortil'])) {
                        ...
                        if ($u == true) {
                            ...
                            if (is_numeric($uin)) {
                                if ($db->query("UPDATE `firms` SET `uid` = '{$uin}' WHERE `id` = '$id' LIMIT 1")) {
                                    ...
                                    sysmel($uin,
                                        '<b>--Firma</b><br>Du har mottat et firma ifra ' . user($obj->id) . '! Klikk her for &aring; se ditt panel over firmaene dine: <a href="http://mafia-no.net/Firmaer">Firmapanel</a>');
                                } else {
                        ...
                        if ($v1 == $current_value1 && $v2 == $current_value2 && $v3 == $current_value3 && $v4 == $current_value4) {
                            ...
                            if (($v1 >= 10000 && $v1 <= 1000000) && ($v2 >= 10 && $v2 <= 60) && ($v3 >= 5 && $v3 <= 30) && ($v4 >= 100 && $v4 <= 1000)) {
                                ...
                                if ($v1 >= 10000 && $v1 <= 1000000) {
                                    echo feil('Tallet m&aring; v&aelig;re mellom 10,000kr-1,000,000kr p&aring; loddprisen!');
                                }
                                if ($v2 >= 10 && $v2 <= 60) {
                                    echo feil('Tallet m&aring; v&aelig;re mellom 10 til 60 minutter p&aring; tiden!');
                                }
                                if ($v3 >= 5 && $v3 <= 30) {
                                    echo feil('Tallet m&aring; v&aelig;re mellom 5%-15% p&aring; prosenten som trekkes av til firmakonto!');
                                }
                                if ($v4 >= 100 && $v4 <= 1000) {
                                    echo feil('Tallet m&aring; v&aelig;re mellom 100-1,000 stk p&aring; antall lodd spillere kan kj&oslash;pe samtidig!');
                                }
                            }
                        }
                    }
