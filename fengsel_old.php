...
if (!(bunker())) {
    ...
    $bu = bunker(true);
    echo '
	<p class="feil">Du er i bunker, gjenstående tid: <span id="bunker">' . $bu . '</span><br>Du er ute kl. ' . date(
            "H:i:s d.m.Y",
            (time() + $bu)
        ) . '</p>
	<script>
    ...
    if (isset($_POST['valget']) && (isset($_POST['kjope']) || isset($_POST['bryte']))) {
        ...
        if (fengsel() == true) {
            ...
            if ($ut == 1) {
                $getjailuser = $db->query(
                    "SELECT * FROM `jail` WHERE `breaker` IS NULL AND `timeleft` > UNIX_TIMESTAMP() AND `id` = '$tid'"
                );
                if ($db->num_rows() == 0) {
                ...
                if ($f->timeleft >= (time() + 600)) {
                    ...
                    if ($obj->hand >= $f->priceout) {
                        if ($db->query(
                            "UPDATE `users` SET `hand` = (`hand` - '{$f->priceout}') WHERE `id` = '{$obj->id}' LIMIT 1"
                        )) {
                            if ($db->query(
                                "UPDATE `jail` SET `breaker` = '{$obj->id}' WHERE `breaker` IS NULL AND `id` = '$tid' AND `timeleft` > UNIX_TIMESTAMP() ORDER BY `id` DESC LIMIT 1"
                            )) {
                                $db->query("UPDATE `jail` SET `breaker` = '{$obj->id}' WHERE `id` = '$tid'");
                                $res = lykket(
                                    'Du har kjøpt ut ' . user($f->uid) . ' for ' . number_format($f->priceout) . 'kr!'
                                );
                        ...
                        if ($f->timeleft >= (time() + 600)) {
                            ...
                            if ($n2 <= 60) {
                                if (settinn($obj->id, "Prøvde å bryte ut {$userlocked}", 60)) {
                                    $res = feil(
                                        'Du klarte ikke å bryte ut ' . user(
                                            $f->uid
                                        ) . '! De oppdaget det og kastet deg like så godt inn.'
                                    );
                                } else {
                                ...
                                if ($db->query(
                                    ...
                                    if ($db->query(
                                        "UPDATE `users` SET `exp` = (`exp` + 0.4) WHERE `id` = '{$obj->id}' LIMIT 1"
                                    )) {
                                        $res = lykket(
                                            'Du klarte å bryte ut ' . user($f->uid) . '!<br>Du tjente 0.4xp!'
                                        );
    ...
    <form method="post" action="">
        <table class="table" style="margin: 0px auto; margin-top: 10px; margin-bottom: 15px;">
            ...
            $timenow = time();
            $s = $db->query(
                "SELECT * FROM `jail` WHERE `breaker` IS NULL AND `timeleft` > UNIX_TIMESTAMP() ORDER BY `timeleft` DESC"
            );
            if ($db->num_rows() >= 1) {
                $numid = 0;
