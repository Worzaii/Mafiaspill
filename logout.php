...
if (isset($_GET['g'])) {
    $g = $_GET['g'];
    if ($g == 1) {
        $_SESSION['grunn'] = '<p class="info">Utlogget på grunn av inaktivitet over 30 minutter.<br>
Logg inn igjen for å fortsette...</p>';
    } elseif ($g == 2) {
        $_SESSION['grunn'] = '<p class="feil">Du ble sendt hit fordi databasen ikke var tilgjengelig,<br>
prøv igjen senere!</p>';
    } elseif ($g == 3) {
        $_SESSION['grunn'] = '<p class="warning">Du ble sendt hit fordi det var noe som<br>
ikke helt stemte med din session!</p>';
    } elseif ($g == 4) {
        $_SESSION['grunn'] = '<p class="feil">Du ble sendt hit fordi brukernavnet og passordet ikke lengre<br>
stemmer overens i forhold til innloggingsdata.</p>';
    } elseif ($g == 5) {
        $_SESSION['grunn'] = '<p class="info">Du ble sendt hit fordi du har vært inaktiv for lenge,<br>
logg inn igjen for å fortsette.</p>';
    } elseif ($g == 6) {
        $_SESSION['grunn'] = '<p class="info">Du ble sendt hit fordi en admin logget deg ut. Da er det mulig du har<br>
