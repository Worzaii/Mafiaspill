...
<table class="table">
<tr>
<th colspan="2">Spillere, og passord:</th><th>Antall like:</th>
</tr>
<?php
$p = $db->query("SELECT  `pass` , COUNT(`pass`) AS  `antp` FROM  `users` WHERE `activated` = '1' AND `status` IN (3,4,5,6) GROUP BY  `pass` HAVING COUNT(`pass`) > 1 ORDER BY `antp` DESC LIMIT 0 , 30");
if($db->num_rows($p) >= 1){
...
echo '</table>';
$nys = $db->query("SELECT `mail`,COUNT(`mail`) AS `ant` FROM `users` WHERE `mail` <> '' GROUP BY `mail` HAVING COUNT(`mail`) > 1 ORDER BY `ant` DESC");
