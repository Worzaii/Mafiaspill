...
include("core.php");
startpage("Flyplass");
echo '<img src="images/headers/flyplass.png">';
?>
    <h1>Flyplass</h1>
<?php
if (fengsel()) {
...
<br>Du er ute kl. ' . date("H:i:s d.m.Y", (time() + $bu))) .
    ...
    $airport = $db->query("select timestamp from flight_log where (timestamp + 600) > unix_timestamp() limit 0,1");
    error_log(var_export($airport, true));
    if ($airport->num_rows == 1) {
        $wait = ($airport->fetch_object()->timestamp + 600) - time();
        error_log($wait);
...
Du kan fly igjen kl: ' . date('H:i:s d.m.Y', time() + $wait));
    } else {
...
values ('{$obj->id}',unix_timestamp(), '$obj->city', '$i', '10000')");
                                error_log("Trying to add to flight log... Result: " . var_export($flightlog, true));
                            } else {
        ...
        ?>
        <p>&aring; ta fly vil koste deg 10,000kr ingame. Dette blir endret i fremtiden slik at firmaeierne kan
