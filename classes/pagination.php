...
class Pagination
{
    ...
    function __construct($db, $sqlText, $per_page, $get_var, $adjacents = 1)
    {
        ...
        $this->sqlResult = $db->query($this->sqlText) or die(
            'Kunne ikke lage sidetall ' . mysqli_error(
                $db->connection_id
            ) . ' <b>Vennligst rapporter til administrator.</b>'
        );
        ...
        // Higher than max
        $this->current_page = $this->current_page > ceil($this->num_rows / $per_page) ? ceil(
            $this->num_rows / $per_page
        )
            : $this->current_page;
    ...
    function GetPageLinks()
    {
        ...
        if ($num_rows <= $this->per_page) {
            ...
            $links .= $begin / $this->per_page <= 1 ? '<span class="prev">Forrige</span> ' : '<a href="' . $this->GetLinkHref(
                    $this->current_page
                    - 1
                ) . '" class="prev">Forrige</a> ';
            ...
            if ($this->GetNumPages() < 7 + ($this->adjacents * 2)) {
                ...
                if ($this->current_page < 1 + ($this->adjacents * 2)) {
                    ...
                    $links .= '<a href="' . $this->GetLinkHref($this->GetNumPages() - 1) . '">' . ($this->GetNumPages(
                            ) - 1) . '</a> '; // last page - 1
                    $links .= '<a href="' . $this->GetLinkHref($this->GetNumPages()) . '">' . ($this->GetNumPages(
                        )) . '</a> '; // last page
                } elseif ($this->GetNumPages(
                    ) - ($this->adjacents * 2) > $this->current_page && $this->current_page > ($this->adjacents
                        * 2)) {
            ...
            $links .= $this->current_page >= $this->GetNumPages(
            ) ? '<span class="next">Neste</span>' : '<a href="' . $this->GetLinkHref(
                    $this->current_page
                    + 1
                ) . '" class="next">Neste</a>';
        }
    ...
    {
        return '<a href="#" onclick="var page = prompt(\'Skriv et sidetall. 1 til ' . $this->GetNumPages(
            ) . '.\'); if( page > 0 &amp;&amp; page &lt;= ' . $this->GetNumPages(
            ) . ' ){ NavigateTo(\'' . $this->GetLinkHref(
                "'+page+'"
            ) . '\'); } return false;" class="seperator">...</a>';
    }
}
