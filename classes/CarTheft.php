...
class CarTheft
{
    ...
    {
        $r1 = $this->database->prepare(
            "SELECT count(*) FROM `carslog` WHERE `uid` = ? AND `timewait` > UNIX_TIMESTAMP() ORDER BY `id` DESC LIMIT 0,1"
        );
        ...
        if ($r1->fetchColumn() == 0) {
            ...
            $r2 = $this->database->prepare(
                "SELECT (timewait - unix_timestamp()) as timeleft from carslog where uid = ? and timewait > unix_timestamp() order by id desc limit 0,1"
            );
            $r2->execute([]);
